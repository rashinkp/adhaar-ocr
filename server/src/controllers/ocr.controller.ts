...
export class OcrController {
  ...
  async processOcr(req: Request, res: Response) {
    try {
      ...
      const { parsed, frontText, backText } = ocrResult as {
        parsed?: {
  ...
  /**
   * GET /search - Find Aadhaar Record
   */
  async findRecord(req: Request, res: Response) {
    ...
    try {
      if (!aadhaarNumber) {
        ...
        res
          .status(400)
          .json({ success: false, message: "aadhaarNumber is required" });
        return;
      }
