...
const sanitizeAddressHead = (address) => {
...
const cleanAddressChunks = (address) => {
    // Start with punctuation normalization then split into chunks
    let chunks = normalizePunctuation(address)
        .split(/\s*,\s*/)
...
const selectAddressLines = (chunks) => {
    if (chunks.length === 0)
        return [];
    ...
    for (let i = (careOfIdx !== -1 ? careOfIdx + 1 : 0); i < chunks.length; i++) {
        const c = chunks[i];
        if (/uidai|gov|help@|india|unique|ident/i.test(c))
            continue;
        if (!/[A-Za-z]{3,}/.test(c))
            continue;
    ...
    if (statePin) {
        ...
        if (m)
            statePin = `${m[1].trim()} - ${m[2]}`;
    }
    const lines = [careOf, locality, statePin].filter((v) => Boolean(v));
    const dedup = [];
    for (const l of lines) {
        const cleaned = l.replace(/^[-,\s]+|[-,\s]+$/g, "").trim();
        if (!cleaned)
            continue;
        if (!dedup.some((x) => lower(x) === lower(cleaned)))
            dedup.push(cleaned);
    }
    return dedup;
};
const formatStatePinInline = (text) => {
...
const calculateConfidence = (parsed, rawText) => {
    let confidence = 0;
    let totalFields = 0;
...
const extractAadhaarNumber = (text) => {
    ...
    const candidates = [];
    const addCandidate = (raw, score, lineIndex) => {
    ...
    lines.forEach((line, i) => {
        ...
        if (/[\/-]/.test(line) && /(dob|yob|date)/i.test(line))
            return;
...
const extractGender = (text) => {
    ...
    if (/\bmale\b/i.test(text))
        return "Male";
    if (/\btransgender\b|\bother\b/i.test(text))
        return "Other";
    return undefined;
};
const isLikelyName = (line) => {
    ...
    if (digitsCount > 2 && digitsCount >= lettersCount)
        return false;
    if (/uidai|gov|help@|india|unique|ident/i.test(line))
        return false;
    if (/address|dob|yob|male|female/i.test(line))
        return false;
    if (/\bc\s*\/\s*o\b/i.test(line))
        return false;
    const letters = line.replace(/[^A-Za-z\s]/g, "");
    return letters.trim().length >= Math.min(line.trim().length * 0.8, line.trim().length);
};
const extractName = (front, back) => {
...
const extractAddress = (text) => {
    ...
    if (labelIndex !== -1) {
        ...
        for (let i = labelIndex + 1; i < Math.min(lines.length, labelIndex + 8); i++) {
            const l = lines[i];
            if (/uidai|gov|help@|india|unique|ident/i.test(l))
                break;
    ...
    const filtered = lines.filter((l) => {
        ...
        const digitsOnly = l.replace(/\D/g, "");
        if (/(\d{12,})/.test(digitsOnly))
            return false; // exclude Aadhaar number lines
        const letters = (l.match(/[A-Za-z]/g) || []).length;
        const nonLetters = l.length - letters;
        if (letters === 0 || nonLetters > letters * 2)
            return false;
        return true;
    });
    let bestBlock = "";
...
export const parseAadhaarText = (frontText, backText) => {
    ...
    if (name)
        result.name = name;
    if (address)
        result.address = sanitizeAddressHead(address);
    return result;
};
export const parseAadhaarTextWithValidation = (frontText, backText) => {
    try {
        const normalizedFront = normalize(frontText);
        const normalizedBack = normalize(backText);
        ...
        const parsed = {};
        if (aadhaarNumber)
            parsed.aadhaarNumber = aadhaarNumber;
        if (dob)
            parsed.dob = dob;
        if (gender)
        ...
        if (name)
            parsed.name = name;
        if (address)
            parsed.address = sanitizeAddressHead(address);
        // Validate each field
        const errors = [];
        const warnings = [];
        const suggestions = [];
...
//# sourceMappingURL=aadhaar.parser.service.js.map